    ...
    (svg) => {
      ...
      var preTestHandle = svg.append("g")
          ...
          .call(d3.drag().on("drag", function(event) {
                  ...
                  var dyNeg = lrXNeg - (preTestHandleY + event.dy);
                  var dyPos = lrXPos - (preTestHandleY + event.dy);
                  ...
                  if(newPostTestYNeg >= rangeMin
                      ...
                      && (preTestHandleY + event.dy) >= rangeMin
                      && (preTestHandleY + event.dy) <= rangeMax
                      ) {
                      preTestHandleY += event.dy;
      ...
      var lRHandleNeg = svg.append("g")
          ...
          .call(d3.drag().on("drag", function(event) {
              ...
              var dy = (lRHandleYNeg + event.dy) - ptY;
              var newPostTestY = (lRHandleYNeg + event.dy) + dy;
              if(newPostTestY >= rangeMin
                  && newPostTestY <= rangeMax
                  && (lRHandleYNeg + event.dy) >= logLikelihoodRatioScale(lLScaleMin)
                  && (lRHandleYNeg + event.dy) <= logLikelihoodRatioScale(lLScaleMax)
                  ) {
                      lRHandleYNeg += event.dy;
      ...
      var lRHandlePos = svg.append("g")
          ...
          .call(d3.drag().on("drag", function(event) {
                  ...
                  //check that both still on scale
                  var ptY = Number(preTestHandleY);
                  var dy = (lRHandleYPos + event.dy) - ptY;
                  var newPostTestY = (lRHandleYPos + event.dy) + dy;
                  if(newPostTestY >= rangeMin
                      && newPostTestY <= rangeMax
                      && (lRHandleYPos + event.dy) >= logLikelihoodRatioScale(lLScaleMin)
                      && (lRHandleYPos + event.dy) <= logLikelihoodRatioScale(lLScaleMax)
                  ) {
                      lRHandleYPos += event.dy;
      ...
      var postTestHandleNeg = svg.append("g")
          ...
          .call(d3.drag().on("drag", function(event) {
              ...
              var newLRy = 0.5 * ((postTestHandleYNeg + event.dy) + Number(preTestHandleY));
              if((postTestHandleYNeg + event.dy) >= rangeMin
                  && (postTestHandleYNeg + event.dy) <= rangeMax
      ...
      var postTestHandlePos = svg.append("g")
          ...
          .call(d3.drag().on("drag", function(event) {
                  //dragging this updates both postTest and lr handles - check that both still on scale
                  var newLRy = 0.5 * ((postTestHandleYPos + event.dy) + Number(preTestHandleY));
                  if((postTestHandleYPos + event.dy) >= rangeMin
                      && (postTestHandleYPos + event.dy) <= rangeMax
                      && newLRy >= logLikelihoodRatioScale(lLScaleMin)
                      && newLRy <= logLikelihoodRatioScale(lLScaleMax)
                  ) {
                      //console.log(newLRy);
                      //console.log(logLikelihoodRatioScale(lLScaleMax));
                      postTestHandleYPos += event.dy;
                      lRHandleYPos = newLRy;
